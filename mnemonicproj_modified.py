...
expInfo = {'participant': '', 'age': '', 'gender(men=1, women=2, others=3)':'',}
...
inandout_dec_options = visual.TextStim(
    ...
    text='H = ambiguous, J = indoor, K = outdoor',
    font='Open Sans',
    pos=(0, -0.35),
    height=0.03,
    wrapWidth=None,
...
The objects you will see may be exactly the same as what you've seen before, but it can be very slightly different, or it can be completely different.
...
for _run in range(1, 5):
    ...
    for thisEncoding_trials_block in encoding_trials_block:
        ...
        # keep track of which components have finished
        encoding_imgComponents = [encoding_img, inandout_dec_options, inandout_dec_resp]
        #encoding_imgComponents = [encoding_img, inandout_dec_instruct, inandout_dec_options, inandout_dec_resp]
        ...
        while continueRoutine and routineTimer.getTime() > 0:
            ...
            if inandout_dec_resp.status == STARTED and not waitOnFlip:
                theseKeys = inandout_dec_resp.getKeys(keyList=['h', 'j', 'k'], waitRelease=False)
        ...
        #encoding_trials_block.addData('inandout_dec_instruct.started', inandout_dec_instruct.tStartRefresh) #might have to delete
        #encoding_trials_block.addData('inandout_dec_instruct.stopped', inandout_dec_instruct.tStartRefresh) #might have to delete if the program doesn't work
        # check responses
    ...
    # update component parameters for each repeat
    sudoku_timer = core.CountdownTimer(300)
    ...
    # keep track of which components have finished
    sudoku_timer_textComponents = [sound_1, sudoku_timer_text]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        ##################################
        # start/stop sound_1
        if sound_1.status == NOT_STARTED and tThisFlip >= 299.0-frameTolerance:
            # keep track of start time/frame for later
        ...
        if sudoku_timer_text.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > sudoku_timer_text.tStartRefresh + 300.0-frameTolerance:
                # keep track of stop time/frame for later
